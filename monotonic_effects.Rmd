...
measurement error and/or we are intereted in the (hypothetical) latent
...
effects, we would still have to deal with only three related regression
coefficients (two for the main effects and one for the interaction), which would
...
not make the assumption that it was originating from the categorization of a
latent continuous variable as typically done in latent variable models [e.g.,
@winship1984; @joreskog1994; @finney2006; @lei2009]. Such a latent variable
assumption may be sensible if the ordinal
predictor represents the chosen categories of a Likert item, for instance, with
the intention to measure a latent psychological construct, but would certainly
not be sensible in some other cases, for instance, if the categories were known
discrete points in time. Depending on the research question and goal of the
analysis, a latent approach may be favorable but this is clearly not always the
case. Instead, we think that both manifest and latent approaches to modeling
ordinal predictors are important and our contribution lies explictly in the area
of manifest approaches.
